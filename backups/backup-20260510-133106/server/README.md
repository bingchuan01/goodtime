...
- **小程序端**：`Authorization: Bearer <token>`（JWT）。用户登录支持：
  - `POST /api/user/login`：微信 code 登录
  - `POST /api/user/login/sms/send`：发送验证码
  - `POST /api/user/login/sms`：手机号验证码登录/注册
  - `POST /api/user/login/password`：账号密码登录
- 默认关闭 `X-User-Id` 旁路（仅当 `ALLOW_X_USER_ID=1` 开启，供开发联调）。
...
- `WX_APPID` / `WX_SECRET`：微信小程序登录凭据（用于 `code2Session`）
- `ALLOW_X_USER_ID`：开发联调旁路，默认关闭；仅值为 `1` 时开启
- `SMS_REGION_ID`：阿里云短信地域，默认 `cn-hangzhou`
- `SMS_ACCESS_KEY_ID` / `SMS_ACCESS_KEY_SECRET`：阿里云短信子账号密钥
- `SMS_SIGN_NAME`：短信签名名称（需审核通过）
- `SMS_TEMPLATE_CODE`：验证码模板 CODE（需审核通过）
- `SMS_TEMPLATE_PARAM_KEY`：模板变量名，默认 `code`

> 说明：若未配置完整 `SMS_*` 参数，`/api/user/login/sms/send` 会进入本地调试模式（不真实下发短信）。
...
| POST | /api/user/login/sms/send | 发送手机验证码，body: `{ phone }` |
| POST | /api/user/login/sms | 手机验证码登录，body: `{ phone, code }` |
| POST | /api/user/login/password | 账号密码登录，body: `{ account, password }` |
